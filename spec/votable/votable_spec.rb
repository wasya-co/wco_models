...
describe Mongoid::Votable do
  ...
  context 'user1 changes vote on post2 from down to up' do
    before :all do
      @post1.vote :voter_id => @user1.id, :value => :up

      @post2.vote :voter_id => @user1.id, :value => :down
      @post2.vote revote: true, :voter_id => @user1.id, :value => :up
      @post2.reload
    end
  ...
  context "user1 unvote on post1" do
    before(:all) do
      @post1.vote(:voter_id => @user1.id, value: :up)
      @post1.vote(:voter_id => @user1.id, value: :up, :unvote => true)
      @post1.vote(:voter_id => @user2.id, value: :down)
      @post1.reload
    end
